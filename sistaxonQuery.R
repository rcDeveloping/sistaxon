...
sistaxon_seek <- sistaxon %>%
        filter(NomePopular == 'Acapu' | NomePopular == 'Acapurana')
...
CITES_443 <- read.csv2('D:/data/endangeredBrazilianPlantSpecies/output/Especies_Ameacadas_BRA.csv', fileEncoding = 'latin1')


sistaxon_seek %>%
        inner_join(CITES_443, by = c('NomeCientifico' = 'nome_cientifico'))
